 ...
 * Formerly the home-grid Lucid tile; now lives in the case study's
 * hackathon aside.
 *
...
export default function PrismStage() {
  ...
  useEffect(() => {
    ...
    if (!gl) return // no WebGL2 — the stage stays quietly dark
    ...
    const compile = (type: number, src: string) => {
      ...
      if (!gl.getShaderParameter(sh, gl.COMPILE_STATUS)) {
        console.error('PrismStage shader:', gl.getShaderInfoLog(sh))
      }
      return sh
    }
    ...
    if (!gl.getProgramParameter(prog, gl.LINK_STATUS)) {
      console.error('PrismStage link:', gl.getProgramInfoLog(prog))
      return
    }
    gl.useProgram(prog)
  ...
  return (
    <div ref={stageRef} className="lcs-prism">
      <canvas
        ref={canvasRef}
        aria-hidden="true"
        style={{
